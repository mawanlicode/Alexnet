...
DataArr read_Data(const char* filename)//读入IQ数据
{
	FILE  *fp=NULL;
	...
	fread((char*)&number_of_data, sizeof(number_of_data), 1, fp);
	number_of_data = ReverseInt(number_of_data);
	//获取训练或测试图像的高度Heigh  
	...
	// 图像数组的初始化
	DataArr imgarr = (DataArr)malloc(sizeof(SignaldataArrS));
	imgarr->dataNum = number_of_data;
